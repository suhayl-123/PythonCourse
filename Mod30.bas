'====================================================================
'  PESCA - Suivi Intervention   |   Module1   (FINAL)
'
'  Builds an HTML table email (bordered, like the approved mockup).
'   - Intro changes by migration status (OK = succes / NOK = echec)
'   - REF SOLA is on its own row, right under Sollicitation TSKC
'   - Uses each TSC's OWN Outlook signature (via .Display)
'
'  Delivery:
'   1) If classic Outlook (COM) is available -> opens a real HTML draft
'      with the TSC's signature appended automatically.
'   2) If not (New-Outlook-only machine) -> copies the HTML table to the
'      clipboard so the TSC pastes it (Ctrl+V) into a new mail. Pasting
'      keeps the table formatting because the clipboard carries HTML.
'====================================================================
Option Explicit
...
Sub SendMigrationMailEx( _
    ...
    ' ---- build the HTML table (order follows the Suivi sheet) ----
    ...
    If Not TryOutlookHTML(subjectText, html) Then
        ...
        MsgBox "Classic Outlook not available on this PC." & vbCrLf & vbCrLf & _
               ...
               "3) Add recipients and subject:" & vbCrLf & vbCrLf & _
               "Subject: " & subjectText, vbInformation, "Paste into a new mail"
    End If
End Sub

' Open a classic-Outlook HTML draft. Returns False if COM unavailable.
' Uses .Display so the TSC's own default signature is appended.
...
' Escape the few characters that would break HTML.
...
' Put HTML on the clipboard so a paste into Outlook keeps the table.
' Uses the Windows "HTML Format" clipboard descriptor.
Private Sub CopyHtmlToClipboard(ByVal htmlFragment As String)
    Dim html As String, cf As String
    Dim hStart As Long, hEnd As Long, fStart As Long, fEnd As Long
    Dim header As String, dummy As String

    ' Build the CF_HTML payload with byte offsets (required by Windows).
    ...
    html = "<html><body><!--StartFragment-->" & htmlFragment & "<!--EndFragment--></body></html>"
...
' Low-level clipboard write registering the "HTML Format" type.
#If VBA7 Then
...
Private Sub PutHtmlOnClipboard(ByVal payload As String)
    ...
    n = UBound(bytes) - LBound(bytes) + 2   ' +1 data, +1 null terminator
